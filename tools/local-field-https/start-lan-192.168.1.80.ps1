...
$configuration = Join-Path $projectRoot 'tools\local-field-https\httpd-lan-192.168.1.80.conf'
# This entrypoint never issues certificates, changes trust or touches old files.
& $ssl verify -CAfile "$tlsRoot\ca.pem" -verify_ip 192.168.1.80 "$tlsRoot\server-192.168.1.80.pem"
if ($LASTEXITCODE -ne 0) { throw 'Existing LAN server certificate failed verification.' }
& $ssl x509 -in "$tlsRoot\server-192.168.1.80.pem" -checkend 3600 -noout
...
if ($Start) {
    $occupied = Get-NetTCPConnection -LocalAddress 192.168.1.80 -LocalPort 8443 -State Listen -ErrorAction SilentlyContinue
