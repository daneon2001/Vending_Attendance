...
describe('Spanish support UI and confirmed receipts', () => {
  ...
  it('retains existing attendance startup and adds lazy support startup without awaiting it', () => {
    ...
    expect(home).toContain('v-if="terminalActive" expand="block" fill="clear" router-link="/support"')
    expect(main).toContain("startSupport: () => import('./support/services')")
    expect(main).toContain('startTerminal: () => edgeSyncService.start()')
    expect(source('app/startup.ts')).toContain('void ports.startSupport().catch')
