  ...
  <ion-page>
    ...
    <ion-content>
      <main class="page-shell">
        ...
        <h1>{{ state.summary?.machineCode ?? 'Máquina sin configurar' }}</h1>
        <div class="status-grid">
          <div class="status-card"><span>Dispositivo</span><strong>{{ deviceStatusLabel(state.summary?.deviceStatus) }}</strong></div>
          <div class="status-card"><span>Sincronización</span><strong>{{ syncPhaseLabel(state.phase) }}</strong></div>
          <div class="status-card"><span>Configuración</span><strong>v{{ state.summary?.configurationVersion ?? '—' }}</strong></div>
          ...
          <div class="status-card"><span>Última confirmación de configuración</span><strong>{{ formattedLastSync }}</strong></div>
        </div>
        ...
        <ion-list inset>
          ...
          <ion-item v-for="employee in filteredEmployees" :key="employee.assignment.uuid" button
            ...
            <ion-label><h2>{{ employee.name }}</h2><p>{{ employee.employee_number }} · {{ assignmentLabel(employee.assignment.type) }}</p></ion-label>
          </ion-item>
          <ion-item v-if="filteredEmployees.length === 0"><ion-label class="muted">No hay empleados autorizados disponibles en este dispositivo.</ion-label></ion-item>
        </ion-list>
      </main>
    </ion-content>
  </ion-page>
