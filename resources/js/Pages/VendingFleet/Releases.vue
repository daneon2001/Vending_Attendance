...
const blockRelease = (item) => {
    if (!window.confirm(`Bloquear ${item.version} #${item.build_number} y detener su rollout?`)) return;
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="space-y-6">
            ...
            <section class="card overflow-x-auto"><table class="w-full min-w-[72rem] text-sm"><thead><tr class="text-left text-soft"><th class="p-3">Plataforma</th><th class="p-3">Canal/targets</th><th class="p-3">Versión/build</th><th class="p-3">Estado</th><th class="p-3">Rollout</th><th class="p-3">Obligatoria</th><th class="p-3">Checksum</th><th class="p-3">Publicada</th><th class="p-3">Acción</th></tr></thead><tbody><tr v-for="item in releases" :key="item.uuid" class="border-t border-app"><td class="p-3">{{ item.platform }}</td><td class="p-3">{{ item.channel }}<br><span class="text-xs text-soft">{{ item.targets?.length ? item.targets.map((value) => `${value.target_type}:${value.target_value}`).join(', ') : 'CHANNEL completo' }}</span></td><td class="p-3 font-mono">{{ item.version }} / {{ item.build_number }}</td><td class="p-3">{{ item.status }}</td><td class="p-3">{{ item.rollout_percentage }}%</td><td class="p-3">{{ item.mandatory ? 'Sí' : 'No' }}</td><td class="p-3 font-mono text-xs">{{ item.artifact_sha256 ? `${item.artifact_sha256.slice(0, 12)}…` : '—' }}</td><td class="p-3">{{ item.released_at || '—' }}</td><td class="p-3"><button v-if="canManage && item.status === 'PUBLISHED'" type="button" class="font-semibold text-rose-600" @click="blockRelease(item)">Bloquear rollout</button><span v-else>—</span></td></tr><tr v-if="!releases.length"><td colspan="9" class="p-5 text-center text-soft">Sin releases registradas.</td></tr></tbody></table></section>
        </div>
    </AuthenticatedLayout>
</template>
