<?php

namespace Tests\Feature\Api;

use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Schema;
use Illuminate\Support\Str;
use Tests\TestCase;

class OnPremHmacAttendanceTest extends TestCase
{
    private bool $createdCompaniesTable = false;
    private bool $createdLocationsTable = false;
    private bool $createdClocksTable = false;
    private bool $createdDevicesTable = false;
    private bool $createdDeviceNoncesTable = false;
    private bool $createdAttendancesRawTable = false;
    private bool $createdAuditLogsTable = false;

    protected function setUp(): void
    {
        parent::setUp();

        config([
            'onprem.hmac_tolerance_seconds' => 300,
            'onprem.nonce_ttl_seconds' => 600,
            'onprem.max_batch_size' => 500,
        ]);

        if (! Schema::hasTable('companies')) {
            Schema::create('companies', function (Blueprint $table): void {
                $table->id();
                $table->string('name');
                $table->string('code', 20)->nullable();
                $table->tinyInteger('status')->default(1);
                $table->timestamps();
            });
            $this->createdCompaniesTable = true;
        }

        if (! Schema::hasTable('locations')) {
            Schema::create('locations', function (Blueprint $table): void {
                $table->id();
                $table->unsignedBigInteger('company_id')->nullable();
                $table->string('name');
                $table->string('code', 30)->nullable();
                $table->string('timezone', 60)->nullable();
                $table->string('address')->nullable();
                $table->tinyInteger('status')->default(1);
                $table->timestamps();
            });
            $this->createdLocationsTable = true;
        }

        if (! Schema::hasTable('clocks')) {
            Schema::create('clocks', function (Blueprint $table): void {
                $table->id();
                $table->unsignedBigInteger('company_id')->nullable();
                $table->unsignedBigInteger('location_id')->nullable();
                $table->string('clock_name');
                $table->string('serial_number')->nullable();
                $table->string('ip_address', 45)->nullable();
                $table->string('type_inout', 50)->nullable();
                $table->tinyInteger('status')->default(1);
                $table->timestamp('last_heartbeat_at')->nullable();
                $table->string('last_status_message')->nullable();
                $table->timestamps();
            });
            $this->createdClocksTable = true;
        }

        if (! Schema::hasTable('devices')) {
            Schema::create('devices', function (Blueprint $table): void {
                $table->id();
                $table->string('device_serial', 120)->unique();
                $table->unsignedBigInteger('clock_id')->nullable();
                $table->unsignedBigInteger('unit_id')->nullable();
                $table->unsignedBigInteger('company_id')->nullable();
                $table->string('shared_secret', 255);
                $table->boolean('is_active')->default(true);
                $table->dateTime('last_seen_at')->nullable();
                $table->timestamps();
            });
            $this->createdDevicesTable = true;
        }

        if (! Schema::hasTable('device_nonces')) {
            Schema::create('device_nonces', function (Blueprint $table): void {
                $table->id();
                $table->unsignedBigInteger('device_id');
                $table->string('nonce', 120);
                $table->dateTime('seen_at');
                $table->dateTime('expires_at');
                $table->timestamps();

                $table->unique(['device_id', 'nonce'], 'device_nonces_device_nonce_unique');
            });
            $this->createdDeviceNoncesTable = true;
        }

        if (! Schema::hasTable('attendances_raw')) {
            Schema::create('attendances_raw', function (Blueprint $table): void {
                $table->bigIncrements('remote_event_id');
                $table->unsignedBigInteger('device_id')->nullable();
                $table->string('device_serial', 120);
                $table->string('local_event_id', 120);
                $table->unsignedBigInteger('collaborator_id');
                $table->unsignedBigInteger('clock_id')->nullable();
                $table->unsignedBigInteger('unit_id')->nullable();
                $table->unsignedBigInteger('company_id')->nullable();
                $table->dateTime('event_time_utc');
                $table->dateTime('event_time_local');
                $table->string('tz', 120);
                $table->string('type', 20);
                $table->string('source', 80)->nullable();
                $table->json('meta')->nullable();
                $table->timestamps();

                $table->unique(['device_serial', 'local_event_id'], 'att_raw_device_local_unique');
            });
            $this->createdAttendancesRawTable = true;
        }

        if (! Schema::hasTable('audit_logs')) {
            Schema::create('audit_logs', function (Blueprint $table): void {
                $table->bigIncrements('id');
                $table->unsignedBigInteger('user_id')->nullable();
                $table->string('user_name')->nullable();
                $table->string('user_email')->nullable();
                $table->string('event', 150);
                $table->string('auditable_type')->nullable();
                $table->unsignedBigInteger('auditable_id')->nullable();
                $table->string('description')->nullable();
                $table->json('metadata')->nullable();
                $table->string('ip_address', 45)->nullable();
                $table->string('user_agent', 500)->nullable();
                $table->timestamps();
            });
            $this->createdAuditLogsTable = true;
        }

        $this->clearTables();
    }

    protected function tearDown(): void
    {
        if ($this->createdAuditLogsTable && Schema::hasTable('audit_logs')) {
            Schema::drop('audit_logs');
        }
        if ($this->createdAttendancesRawTable && Schema::hasTable('attendances_raw')) {
            Schema::drop('attendances_raw');
        }
        if ($this->createdDeviceNoncesTable && Schema::hasTable('device_nonces')) {
            Schema::drop('device_nonces');
        }
        if ($this->createdDevicesTable && Schema::hasTable('devices')) {
            Schema::drop('devices');
        }
        if ($this->createdClocksTable && Schema::hasTable('clocks')) {
            Schema::drop('clocks');
        }
        if ($this->createdLocationsTable && Schema::hasTable('locations')) {
            Schema::drop('locations');
        }
        if ($this->createdCompaniesTable && Schema::hasTable('companies')) {
            Schema::drop('companies');
        }

        parent::tearDown();
    }

    public function test_attendances_accept_valid_hmac_signature(): void
    {
        $fixture = $this->seedDeviceFixture('SERIAL-ONPREM-001', 'super-secret-001');
        $payload = [
            'device_serial' => $fixture['device_serial'],
            'clock_id' => $fixture['clock_id'],
            'unit_id' => $fixture['unit_id'],
            'company_id' => $fixture['company_id'],
            'timezone' => 'America/Mexico_City',
            'punches' => [
                [
                    'local_event_id' => '11111111-1111-1111-1111-111111111111',
                    'collaborator_id' => 1001,
                    'event_time_utc' => '2026-02-18T18:35:00Z',
                    'event_time_local' => '2026-02-18T12:35:00-06:00',
                    'tz' => 'America/Mexico_City',
                    'type_inout' => 'IN',
                    'source' => 'FINGERPRINT_MATCH',
                    'meta' => ['quality' => 97],
                ],
                [
                    'local_event_id' => '22222222-2222-2222-2222-222222222222',
                    'collaborator_id' => 1001,
                    'event_time_utc' => '2026-02-18T22:45:00Z',
                    'event_time_local' => '2026-02-18T16:45:00-06:00',
                    'tz' => 'America/Mexico_City',
                    'type_inout' => 'OUT',
                    'source' => 'FINGERPRINT_MATCH',
                    'meta' => ['quality' => 95],
                ],
            ],
        ];

        $response = $this->signedJsonRequest(
            method: 'POST',
            uri: '/api/onprem/attendances',
            payload: $payload,
            deviceSerial: $fixture['device_serial'],
            secret: $fixture['secret'],
        );

        $response->assertOk()
            ->assertJsonPath('ok', true)
            ->assertJsonCount(2, 'received')
            ->assertJsonCount(0, 'rejected');

        $remoteId = $response->json('received.0.remote_event_id');
        $this->assertNotNull($remoteId);

        $this->assertDatabaseHas('attendances_raw', [
            'remote_event_id' => $remoteId,
            'device_serial' => $fixture['device_serial'],
            'local_event_id' => '11111111-1111-1111-1111-111111111111',
        ]);

        $this->assertSame(2, DB::table('audit_logs')->where('event', 'onprem.punch.received')->count());
    }

    public function test_attendances_reject_invalid_signature(): void
    {
        $fixture = $this->seedDeviceFixture('SERIAL-ONPREM-002', 'super-secret-002');
        $payload = [
            'device_serial' => $fixture['device_serial'],
            'clock_id' => $fixture['clock_id'],
            'unit_id' => $fixture['unit_id'],
            'company_id' => $fixture['company_id'],
            'timezone' => 'America/Mexico_City',
            'punches' => [
                [
                    'local_event_id' => '33333333-3333-3333-3333-333333333333',
                    'collaborator_id' => 2001,
                    'event_time_utc' => '2026-02-18T20:00:00Z',
                    'event_time_local' => '2026-02-18T14:00:00-06:00',
                    'tz' => 'America/Mexico_City',
                    'type_inout' => 'IN',
                    'source' => 'FINGERPRINT_MATCH',
                    'meta' => ['quality' => 90],
                ],
            ],
        ];

        $response = $this->signedJsonRequest(
            method: 'POST',
            uri: '/api/onprem/attendances',
            payload: $payload,
            deviceSerial: $fixture['device_serial'],
            secret: 'wrong-secret',
        );

        $response->assertStatus(401)
            ->assertJsonPath('message', 'Invalid signature.');

        $this->assertSame(0, DB::table('attendances_raw')->count());
    }

    public function test_attendances_are_idempotent_per_device_and_local_event_id(): void
    {
        $fixture = $this->seedDeviceFixture('SERIAL-ONPREM-003', 'super-secret-003');
        $payload = [
            'device_serial' => $fixture['device_serial'],
            'clock_id' => $fixture['clock_id'],
            'unit_id' => $fixture['unit_id'],
            'company_id' => $fixture['company_id'],
            'timezone' => 'America/Mexico_City',
            'punches' => [
                [
                    'local_event_id' => '44444444-4444-4444-4444-444444444444',
                    'collaborator_id' => 3001,
                    'event_time_utc' => '2026-02-18T21:00:00Z',
                    'event_time_local' => '2026-02-18T15:00:00-06:00',
                    'tz' => 'America/Mexico_City',
                    'type_inout' => 'INOUT',
                    'source' => 'FINGERPRINT_MATCH',
                    'meta' => ['quality' => 88],
                ],
            ],
        ];

        $first = $this->signedJsonRequest(
            method: 'POST',
            uri: '/api/onprem/attendances',
            payload: $payload,
            deviceSerial: $fixture['device_serial'],
            secret: $fixture['secret'],
        );
        $first->assertOk();

        $second = $this->signedJsonRequest(
            method: 'POST',
            uri: '/api/onprem/attendances',
            payload: $payload,
            deviceSerial: $fixture['device_serial'],
            secret: $fixture['secret'],
        );
        $second->assertOk();

        $firstRemoteId = $first->json('received.0.remote_event_id');
        $secondRemoteId = $second->json('received.0.remote_event_id');

        $this->assertSame($firstRemoteId, $secondRemoteId);
        $this->assertSame(
            1,
            DB::table('attendances_raw')
                ->where('device_serial', $fixture['device_serial'])
                ->where('local_event_id', '44444444-4444-4444-4444-444444444444')
                ->count()
        );
    }

    public function test_ping_accepts_valid_hmac_signature(): void
    {
        $fixture = $this->seedDeviceFixture('SERIAL-ONPREM-004', 'super-secret-004');

        $response = $this->signedJsonRequest(
            method: 'GET',
            uri: '/api/onprem/ping',
            payload: [],
            deviceSerial: $fixture['device_serial'],
            secret: $fixture['secret'],
        );

        $response->assertOk()
            ->assertJsonPath('ok', true);

        $this->assertNotNull($response->json('server_time_utc'));
    }

    public function test_heartbeat_accepts_valid_hmac_and_writes_audit_log(): void
    {
        $fixture = $this->seedDeviceFixture('SERIAL-ONPREM-005', 'super-secret-005');
        $payload = [
            'device_serial' => $fixture['device_serial'],
            'clock_id' => $fixture['clock_id'],
            'unit_id' => $fixture['unit_id'],
            'company_id' => $fixture['company_id'],
            'app_version' => '1.5.0',
            'api_ok' => true,
            'device_ok' => true,
            'pending_count' => 2,
            'last_event_at_utc' => '2026-02-18T21:10:00Z',
            'last_event_at_local' => '2026-02-18T15:10:00-06:00',
            'tz' => 'America/Mexico_City',
            'ip_local' => '192.168.1.20',
            'port' => 8080,
            'status_message' => 'Healthy',
        ];

        $response = $this->signedJsonRequest(
            method: 'POST',
            uri: '/api/onprem/heartbeat',
            payload: $payload,
            deviceSerial: $fixture['device_serial'],
            secret: $fixture['secret'],
        );

        $response->assertOk()
            ->assertJsonPath('ok', true);

        $this->assertSame(1, DB::table('audit_logs')->where('event', 'onprem.heartbeat.received')->count());

        $lastSeen = DB::table('devices')
            ->where('device_serial', $fixture['device_serial'])
            ->value('last_seen_at');

        $this->assertNotNull($lastSeen);
    }

    private function clearTables(): void
    {
        foreach (['device_nonces', 'attendances_raw', 'devices', 'audit_logs', 'clocks', 'locations', 'companies'] as $table) {
            if (Schema::hasTable($table)) {
                DB::table($table)->delete();
            }
        }
    }

    private function seedDeviceFixture(string $deviceSerial, string $secret): array
    {
        $companyId = DB::table('companies')->insertGetId([
            'name' => 'Company Test',
            'code' => 'COMP',
            'status' => 1,
            'created_at' => now(),
            'updated_at' => now(),
        ]);

        $unitId = DB::table('locations')->insertGetId([
            'company_id' => $companyId,
            'name' => 'Unit Test',
            'code' => 'UNIT',
            'timezone' => 'America/Mexico_City',
            'address' => 'Test address',
            'status' => 1,
            'created_at' => now(),
            'updated_at' => now(),
        ]);

        $clockId = DB::table('clocks')->insertGetId([
            'company_id' => $companyId,
            'location_id' => $unitId,
            'clock_name' => 'Clock Test',
            'serial_number' => $deviceSerial,
            'ip_address' => '127.0.0.1',
            'type_inout' => 'INOUT',
            'status' => 1,
            'created_at' => now(),
            'updated_at' => now(),
        ]);

        DB::table('devices')->insert([
            'device_serial' => $deviceSerial,
            'clock_id' => $clockId,
            'unit_id' => $unitId,
            'company_id' => $companyId,
            'shared_secret' => $secret,
            'is_active' => true,
            'created_at' => now(),
            'updated_at' => now(),
        ]);

        return [
            'device_serial' => $deviceSerial,
            'secret' => $secret,
            'clock_id' => $clockId,
            'unit_id' => $unitId,
            'company_id' => $companyId,
        ];
    }

    private function signedJsonRequest(
        string $method,
        string $uri,
        array $payload,
        string $deviceSerial,
        string $secret,
        ?int $timestamp = null,
        ?string $nonce = null,
    ) {
        $method = strtoupper($method);
        $timestamp = $timestamp ?? now()->timestamp;
        $nonce = $nonce ?? (string) Str::uuid();
        $body = in_array($method, ['GET', 'HEAD'], true)
            ? ''
            : json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);

        if ($body === false) {
            $this->fail('Unable to encode JSON payload.');
        }

        $signature = $this->buildSignature(
            method: $method,
            path: $uri,
            timestamp: (string) $timestamp,
            nonce: $nonce,
            body: $body,
            secret: $secret,
        );

        return $this->call(
            $method,
            $uri,
            [],
            [],
            [],
            [
                'HTTP_ACCEPT' => 'application/json',
                'CONTENT_TYPE' => 'application/json',
                'HTTP_X_DEVICE_SERIAL' => $deviceSerial,
                'HTTP_X_TIMESTAMP' => (string) $timestamp,
                'HTTP_X_NONCE' => $nonce,
                'HTTP_X_SIGNATURE' => $signature,
            ],
            $body
        );
    }

    private function buildSignature(
        string $method,
        string $path,
        string $timestamp,
        string $nonce,
        string $body,
        string $secret,
    ): string {
        $bodyHash = hash('sha256', $body);
        $canonical = $method."\n".$path."\n".$timestamp."\n".$nonce."\n".$bodyHash;

        return base64_encode(hash_hmac('sha256', $canonical, $secret, true));
    }
}
